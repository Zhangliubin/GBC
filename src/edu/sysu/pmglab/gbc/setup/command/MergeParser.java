...
class MergeParser {
    ...
    public final CommandOption<File[]> merge;
    ...
    public static MergeParser parse(String... args) {
        return new MergeParser(args);
    }

    public static MergeParser parse(File argsFile) throws IOException {
        return new MergeParser(CommandParser.readFromFile(argsFile));
    }
    ...
    static {
        PARSER.setProgramName("merge <input(s)> -o <output>");
        PARSER.offset(0);
        PARSER.debug(true);
        PARSER.usingAt(true);
        ...
        group001.register(FILE.ARRAY, "merge")
                .arity(-1)
